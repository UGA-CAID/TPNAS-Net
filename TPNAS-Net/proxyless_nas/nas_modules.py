...
class ProxylessNASNets(BasicUnit):
    ...
    def __init__(self, first_conv, blocks, feature_mix_layer, classifier):
        ...
        self.blocks = nn.ModuleList(blocks)  # module list
        self.feature_mix_layer = feature_mix_layer
        # adaptive average pooling,output_size=[HxW] or [HxH]
    ...
    def forward(self, x):
        ...
        gap_embed = self.global_avg_pooling(x)  # change
        x = gap_embed.view(gap_embed.size(0), -1)  # flatten
        x = self.classifier(x)
        return x, gap_embed
